...
public class ChainHashMap<K, V> extends AbstractHashMap<K, V> {
	...
	protected V bucketPut(String hb, K k, V v, int counter) {
		...
		// 32bite tamamlama yapýldý.
		// Daha sonra decimala cevrilip yeri bulundu.
		hb = to32bit(hb);
		...
		// #1// dolu olduðunda ilk locali deðiþebilecek varmý diye bakýldý.
		if (bucket.size() >= 10) {
			...
			while (bucket.size() >= 10 && Removablekey != null && ctemp < 10) {
				...
				// once kopyalar silindi daha sonra yeni global ile eklendi.
				...
				// **RECURSIVE** olusturularak local depth guncellendi.
				oldLocaldepth = 0;
			...
			// #2// en son yapýlacak ihtimal.
			// resize durumu kontrol edildi.
			if (bucket.size() >= 10) {
				globalDepth++;
		...
		V answer;
		// localdepth deðiþmemesi saðlandý(resizeda)
		if (oldLocaldepth != 0) {
