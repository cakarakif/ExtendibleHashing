...
public abstract class AbstractHashMap<K, V> extends AbstractMap<K, V> {
	...
	protected void resize(int newCap) {
		...
		for (Entry<K, V> e : buffer) {
			...
			// depth farklýlýklarýnda tekrar yerletirme yapýldý.
			// local depthler deðiþtirilmeden.
			if (e.getLocaldepth() != globalDepth) {
				String br = codeHash(e.getKey()), or = "";
				br = to32bit(br);
				oldLocaldepth = e.getLocaldepth();

				// globaldeptin 1 eksiði kadarýný alýp sonra tekrar yerleþtir.
				// bi eksiði eþit olup son biti deðiþebilir diye.
	...
	// TXTden girdiler okundu ve fuctionHash'e gönderildi.
	public void readFile() {
		BufferedReader in = null;
		// Try-Catch Kullanýldý.
		try {
	...
	// Binarye çevirilip hash map için gönderildi.
	protected String codeHash(K key) {
		...
		// alýnan sayý binary olarak gönderildi.
		return binary;
	}
